...
| Postgres scenario fails: gateway leaf `certificate has expired` (UI mislabels it "SPIFFE ID not allow-listed") | **SWA v1.0.2 bug**: X.509 SVID rotation wedges — server logs `subscriber already exists for: id=<pid>` (subscriber keyed by the hostPID-stable PID; not released before the workload's rotation reconnect). Only the long-lived pg-gateway is hit; when the Firefly CA rotates, the webapp then fails to verify the gateway leaf against "firefly root". | Mitigated: `x509_workload_ttl`=8h (terraform-swa) + the `swa-svid-resync` CronJob (`k8s/swa-svid-resync.yaml`) proactively rolls the whole chain (swa-server → pg-gateway → swa-demo-webapp) every 2h across the 9-5 ET on-window. To clear a live wedge on demand: `make swa-resync` (issuer → gateway → webapp, in order; a gateway-only bounce is NOT enough for the CA-rotation case). Report the subscriber-cleanup bug to CyberArk. |
