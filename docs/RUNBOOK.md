...
| Postgres scenario fails: gateway leaf `certificate has expired` (UI mislabels it "SPIFFE ID not allow-listed") | **SWA v1.0.2 bug** (historical): X.509 SVID rotation wedged — server logged `subscriber already exists for: id=<pid>` (subscriber keyed by the hostPID-stable PID; not released before the workload's rotation reconnect); the long-lived pg-gateway then served an expired leaf. See `docs/SWA-SVID-ROTATION-BUG.md`. | Addressed by the **SWA v1.0.3 upgrade**, which also let us drop the agent's `hostPID`/`hostNetwork` (a stable host PID was the root of the subscriber collision). The proactive resync CronJob and the manual `make swa-resync` workaround have been **removed**. If a wedge recurs on v1.0.3, restart the chain in order (swa-server → pg-gateway → swa-demo-webapp) and reopen the bug with CyberArk. |
