...
- ✅ **Phase 6** — the interpolated *in-card detail sentences*: node `body` prose (Kv labels, foot, danger notes), layer `pass`/`reject`/`idle` status lines, node state `tag`s (shared `chrome.state.*` vocabulary), the shared `CredentialBody`, and all remaining `buildTrace` `comment` lines (CP's two extra lines + CCP/SWA/Conjur). Interpolation uses `t(key, vars)` with `{placeholder}` tokens. SPA catalogs reached **524 keys**, exact parity. `cmd`/`cont`/`out`/`ok`/`err` trace lines stay verbatim per the glossary.

**SPA localization complete.** Every user-facing string across the landing page and the inspector SPA (shell + views) is localized in en / es-419 / pt-BR, guarded by parity tests on both stacks.
